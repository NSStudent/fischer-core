...
final class MoveTests {
    ...
    func initWithSanMoveAndBoard() async throws {
        ...
        let move = try Move(game: game, sanMove: sanMove, turn: .white)
        #expect(move.start == .e2)
        #expect(move.end == .e4)
        
        #expect(throws: FischerCoreError.illegalMove) {
            let sanIllegalMove = try SanMoveParser().parse("e6")
            let _ = try Move(game: game, sanMove: sanIllegalMove, turn: .white)
        }
        ...
        let move2 = try Move(game: game2, sanMove: sanMove2, turn: .white)
        ...
        let move3 = try Move(game: game3, sanMove: sanMove3, turn: .white)
        ...
        #expect(throws: FischerCoreError.illegalMove) {
            ...
            let _ = try Move(game: game, sanMove: sanIllegalMove2, turn: .white)
        }
        
        let sanMove5 = try SanMoveParser().parse("O-O")
        let move5 = try Move(game: game, sanMove: sanMove5, turn: .white)
        #expect(move5.start == .e1)
        #expect(move5.end == .g1)
        
        let sanMove6 = try SanMoveParser().parse("O-O-O")
        let move6 = try Move(game: game, sanMove: sanMove6, turn: .white)
        #expect(move6.start == .e1)
        #expect(move6.end == .c1)
        
        
        let sanMove7 = try SanMoveParser().parse("O-O")
        let move7 = try Move(game: game, sanMove: sanMove7, turn: .black)
        #expect(move7.start == .e8)
        #expect(move7.end == .g8)
        
        let sanMove8 = try SanMoveParser().parse("O-O-O")
        let move8 = try Move(game: game, sanMove: sanMove8, turn: .black)
        #expect(move8.start == .e8)
        #expect(move8.end == .c8)
        
        let sanMove9 = try SanMoveParser().parse("Ne2d4")
        let move9 = try Move(game: game3, sanMove: sanMove9, turn: .white)
