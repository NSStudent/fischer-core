...
final class MoveTreePGNReaderTests {
    ...
    @Test("Reader returns nil for empty PGN noise")
    func readerReturnsNilForEmptyPGNNoise() async throws {
        let input =
        """
        \u{FEFF}
        % Generated by exporter
        ; No games in this document
        ...
        #expect(try reader.readGame() == nil)
        #expect(try reader.readGames().isEmpty)
    }

    @Test("Reader parses metadata comments and zero castling")
    func readerParsesMetadataCommentsAndZeroCastling() async throws {
        let input =
        #"""
        [Event "Reader coverage"]
        [CustomTag "escaped \"quote\" and \\ slash"]
        [Result "0-1"]
        ...
        var reader = MoveTreePGNReader(input)
        let pgn = try #require(try reader.readGame())

        #expect(pgn.tags[.event] == "Reader coverage")
        #expect(pgn.tags[.custom("CustomTag")] == #"escaped "quote" and \ slash"#)
        #expect(pgn.initialComment == [.text("Initial note")])
        #expect(pgn.result == .loss)
        #expect(pgn.tree?.mainLineDescriptions == ["e4", "e5", "Nf3", "Nc6", "Bc4", "O-O"])
    }

    @Test("Reader reports malformed tags and variations")
    func readerReportsMalformedTagsAndVariations() async throws {
        #expect(throws: MoveTreePGNReader.ReaderError.unterminatedTag) {
            var reader = MoveTreePGNReader(#"[Event "Broken"#)
            _ = try reader.readGame()
        }

        #expect(throws: MoveTreePGNReader.ReaderError.unexpectedVariation) {
            var reader = MoveTreePGNReader(
                """
                [Event "Unexpected variation"]
                [Result "*"]
        ...
        #expect(throws: MoveTreePGNReader.ReaderError.unterminatedVariation) {
            var reader = MoveTreePGNReader(
                """
                [Event "Unterminated variation"]
                [Result "*"]
